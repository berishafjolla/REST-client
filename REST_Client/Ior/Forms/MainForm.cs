...
namespace Swensen.Ior.Forms
{
    public partial class MainForm : Form
    {
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            { //n.b. exceptions swallowed during main load since gui message pump not started
        ...
        {
            BasicAuthentication dialog = new BasicAuthentication();
        ...
        {
            DataGridView grid = this.requestQueryStringParamsGrid;
            var pairs = new List<string>();
            foreach (DataGridViewRow row in grid.Rows)
            {
                string key = (string)row.Cells[0].Value;
                string value = (string)row.Cells[1].Value;
                var combined = string.Join("=", key, value);
                if (string.IsNullOrWhiteSpace(key) == false)
                    pairs.Add(combined);
            }
            var valueFromUrl = this.txtRequestUrl.Text;
            if (string.IsNullOrWhiteSpace(valueFromUrl) == false)
            {
                try
                {
                    var requestUri = new Uri(valueFromUrl);
                    this.txtRequestUrl.Text = requestUri.GetLeftPart(UriPartial.Path) + "?" + string.Join("&", pairs);
                }
                catch (Exception ex)
                {
                    valueFromUrl = "http://" + valueFromUrl;
                    var requestUri = new Uri(valueFromUrl);
                    this.txtRequestUrl.Text = requestUri.GetLeftPart(UriPartial.Path) + "?" + string.Join("&", pairs);
                }
            }
        ...
        public void AddHeader(string header, string value)
        {
            ...
            var regex = new Regex("^" + header + "[^(" + this.txtRequestHeaders.EndOfLine.EolString + ")]+", RegexOptions.Multiline);
            if (regex.IsMatch(this.txtRequestHeaders.Text))
            {
                this.txtRequestHeaders.Text = regex.Replace(this.txtRequestHeaders.Text, header + ": " + value);
                return;
            }
            this.txtRequestHeaders.Text += (this.txtRequestHeaders.Text.Length > 0 ? this.txtRequestHeaders.EndOfLine.EolString : "") + header + ": " + value;
        }
        ...
        private void bind(RequestViewModel requestVm)
        {
            ...
            var uri = new Uri(requestVm.Url);
            var keyValuePair = requestVm.Url.IndexOf('?') != -1 && requestVm.Url.Split('?').Count() > 1 ? HttpUtility.ParseQueryString(requestVm.Url.Split('?')[1]) : new NameValueCollection();
            foreach (var key in keyValuePair.AllKeys)
            {
                var value = keyValuePair[key];
                this.requestQueryStringParamsGrid.Rows.Add(key, value);
            }
